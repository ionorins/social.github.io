...
function addToBasket() {
    ...
    if (newQuantity > 0) {
        ...
        let text = `Added ${newQuantity} ${productDetails[productID].name} to basket!`;
        let elem = document.querySelector("#addBsk");
        let bsk = document.querySelector(".nav-link.basket");

        elem.style.display = "block";
        bsk.style.backgroundColor = "rgb(180, 180, 180)";
        document.querySelector("#addBsk p").innerHTML = text;

        setTimeout(() => {
            elem.style.display = "none";
            bsk.style.backgroundColor = "transparent";
        }, 3000);
